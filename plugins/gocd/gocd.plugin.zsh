...
function clean-agents() {
    alias http="http --ignore-stdin --check-status"
    ...
    http https://delivery.balance.zalan.do/go/api/agents "${auth}" | \
        ...
        while read agent; do
            echo "Deleting $agent";
            http DELETE https://delivery.balance.zalan.do/go/api/agents/${agent} "${auth}" Accept:application/vnd.go.cd.v2+json &> /dev/null &
        done
